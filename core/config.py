...
class ConfigManager:
    ...
    def _validate_config(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "show_on_fullscreen" in data and isinstance(
            data["show_on_fullscreen"], bool
        ):
        ...
        if "enabled_plugins" in data and isinstance(data["enabled_plugins"], list):
            config["enabled_plugins"] = [
                str(p) for p in data["enabled_plugins"] if isinstance(p, str)
            ]

        if "disabled_plugins" in data and isinstance(data["disabled_plugins"], list):
            config["disabled_plugins"] = [
                str(p) for p in data["disabled_plugins"] if isinstance(p, str)
            ]
